...
class REMMaker(object):
    ...
    def get_river_centerline(self):
        ...
        import osmnx as ox
        self.rivers = ox.features_from_bbox((self.bbox[3], self.bbox[1], self.bbox[2], self.bbox[0]), 
                                     tags={'waterway': ['river', 'stream', 'tidal channel']})
    ...
    def get_river_elev(self):
        ...
        # Call Rasterize with direct keyword arguments
        gdal.Rasterize(
            self.centerline_ras,   # output raster filename
            self.river_shp,        # input shapefile (can be path string)
            format='GTiff',
            outputType=gdal.GDT_Byte,
            attribute='id',
            outputBounds=self.extent,
            xRes=self.cell_w,
            yRes=abs(self.cell_h),  # Make sure it's positive
            initValues=0,
            noData=0
        )
